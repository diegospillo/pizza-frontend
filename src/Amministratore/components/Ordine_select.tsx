      ...
      <>   
          {data.map((item:any) => {
            ...
            >
              ...
              {set_pizze(data.indexOf(item),pizze,data).map((item1:any) => (
    ...
    var cont:any = [];
     for(let x=0;x<data.length;x++){
      if(data[val].nome==data[x].nome){
        if(!pizze.includes(pizza[x])){
        pizze.push(pizza[x]);
        cont[pizze.indexOf(pizza[x])]=1;
        }
        else{
            cont[pizze.indexOf(pizza[x])]++;
        }
      }
    }
    var new_pizze:any = [];
    for(let i=0;i<pizze.length;i++){
        new_pizze[i]=pizze[i]+" (X"+cont[i]+")";
    }
    return new_pizze;
  }
